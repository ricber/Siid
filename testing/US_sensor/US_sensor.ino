...
loop()
{
    distprec = dist; // we save the precedent distance in order to calculate the
                     // velocity bellow
    ...
    if ((abs(distprec - dist) < 200)
        && (dist < 2000)) // 200 is a value we have to set uo !!!!
    {
        Serial.print("Distance en cm : ");
        Serial.println(dist);
        vit =
            (distprec - dist) / (DELAY_VIT_CALCULATION
                                 / 1000); // calcul of the approaching velocity
